...
module.exports = {
  ...
  getDashboardDataOfStudent: async (req, res, next) => {
    try {
      const studentId = req.body.user_id;
      if (!studentId) throw createHttpError.BadRequest("Invalid studentId");
      const data = await dashboardServices.getStudentDashboardData(studentId);
      return res.status(200).json({
        success: true,
        message: "dashboard data fetched.",
        data
